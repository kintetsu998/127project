...
$lock(queryResult, true); // freeze the object;
...
function Task(ctx, tag, isTX) {
     ...
     * a function, an error is thrown: `Invalid factory function specified.`
    ...
    this.sequence = this.queue = function (factory, noTracking, cb) {
        return npm.utils.sequence.call(this, factory, noTracking, cb);
    };
...
function $main(options) {
    ...
    $lock(inst, true, options); // freeze the library instance;
 ...
 * @readonly
 */
$main.version = new Version(1, 11, 0);
$lock($main.version, true);// freeze version object;
...
$lock($main.end); // lock to read-only access;
...
$lock($main.as, true); // freeze the namespace;
...
$lock($main); // lock to read-only access;
